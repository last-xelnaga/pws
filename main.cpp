    ...
    int direction)
{
    ...
    if (res == 0)
    {
        if (sprintf (file_name, "/sys/class/gpio/gpio%d/direction", gpio) < 0)
        {
            printf ("fprintf failed %s\n", strerror (errno));
            res = 1;
        }
    ...
    if (res == 0)
    {
        ...
        char* p_direction = direction_out;

        if (direction != OUTPUT)
            p_direction = direction_in;

        if (fprintf (fp, "%s", p_direction) < 0)
        {
            printf ("fprintf failed %s\n", strerror (errno));
            res = 1;
        }
    ...
    int level)
{
    ...
    if (res == 0)
    {
        if (sprintf (file_name, "/sys/class/gpio/gpio%d/value", gpio) < 0)
        {
            printf ("fprintf failed %s\n", strerror (errno));
            res = 1;
        }
        ...
        void)
{
    ...
    //unsigned char dumData = bcm2835_spi_transfer(send_data);

  //send_data = 0x80;
  //unsigned char msb = bcm2835_spi_transfer(send_data);
  //int msbRead = msb & 0b00000011

  //send_data = 0;
  //unsigned char lsb = bcm2835_spi_transfer(send_data);

    /*sensor_dht11_class dht11 (settings_class::get_instance ().dht_termo_gpio ());
    while (1)
    {
